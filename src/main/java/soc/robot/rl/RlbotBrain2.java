...
/* funkcje do zmiany:
 * [done] rollOrPlayKnightOrExpectDice (to się przyda: 
 * 			soc.game.SOCPlayerNumbers (This class tracks what dice-roll numbers a player's pieces are
 * 						 touching, and the probability of getting resources based on their hex types. )
 * 			SOCNumberProbabilities.INT_VALUES (jak często wypada dana liczba)
 * 			ports[portType] = (board.getPortCoordinates(portType).contains(firstNodeInt));
 * [done] placeIfExpectPlacing
 * [done] moveRobber()
 * [done][nie trzeba było zmieniać, to tylko dla scenariuszu jednego] planAndPlaceInvItem()
 * [done][not needed handled by buildOrTradeOrPlayCard] playKnightCardIfShould()
 * [done][not needed handled by buildOrTradeOrPlayCard] planBuilding();
 * [done]buildOrGetResourceByTradeOrCard()
 * [done]client.discard(game, DiscardStrategy.discard
                            (((SOCDiscardRequest) mes).getNumberOfDiscards(), buildingPlan, rand,
                              ourPlayerData, robotParameters, decisionMaker, negotiator));
 * [done]RobberStrategy.chooseRobberVictim
 *   (((SOCChoosePlayerRequest) mes).getChoices(), game, playerTrackers);
 * 
 * inne: do one look ahead, trackers mają genialną funkcję: tryPutPiece;
 */

/**
 * AI for playing Settlers of Catan.
 * Represents a robot player trained by reinforcement learning within 1 game.
...
//                    	System.out.println("finished discarding");
                    ...
                    case SOCMessage.CHOOSEPLAYERREQUEST:
                        {
                            final int choicePl = moveRobber[1];
                            counter = 0;
...
//        rlStrategy = new RLStrategyLookupTable(this);
    ...
    protected void buildOrTradeOrPlayCard()
    	...
    	case RLStrategy.PLAY_ROADS:
    		...
    		int[] roadsToBuildCoord = action.getValue();
    		
    		whatWeWantToBuild = new SOCRoad(ourPlayerData, roadsToBuildCoord[0], null);
    		SOCPossiblePiece secondRoad = new SOCPossibleRoad(ourPlayerData, roadsToBuildCoord[1], null);
    		buildingPlan.push(secondRoad);
    	...
    	case RLStrategy.PLACE_SETTLEMENT:
            ...
            counter = 0;
            expectPLACING_SETTLEMENT = true;
            whatWeWantToBuild = new SOCSettlement(ourPlayerData, settlementToBuildCoord[0] , null);
            if (! whatWeWantToBuild.equals(whatWeFailedToBuild))
            {
                D.ebugPrintln("!!! BUILD REQUEST FOR A SETTLEMENT " + Integer.toHexString(settlementToBuildCoord[0]) + " !!!");
                client.buildRequest(game, SOCPlayingPiece.SETTLEMENT);
            } else {
                // We already tried to build this.
                cancelWrongPiecePlacementLocal(whatWeWantToBuild);
...
//    		System.out.println(game.getName() +"." + ourPlayerNumber + ".brain. BeforeplaceS has city pieces " + ourPlayerData.getNumPieces(SOCPlayingPiece.CITY));
    	...
    	case RLStrategy.PLACE_ROAD:
            ...
            counter = 0;
            expectPLACING_ROAD = true;
            whatWeWantToBuild = new SOCRoad(ourPlayerData, roadToBuildCoord[0], null);
            if (! whatWeWantToBuild.equals(whatWeFailedToBuild))
            {
                D.ebugPrintln("!!! BUILD REQUEST FOR A ROAD AT " + Integer.toHexString(roadToBuildCoord[0]) + " !!!");
                client.buildRequest(game, SOCPlayingPiece.ROAD);
            } else {
                // We already tried to build this.
                cancelWrongPiecePlacementLocal(whatWeWantToBuild);
    	...
    	case RLStrategy.PLACE_CITY:
            ...
            counter = 0;
            expectPLACING_CITY = true;
            whatWeWantToBuild = new SOCCity(ourPlayerData, cityToBuildCoord[0], null);
            if (! whatWeWantToBuild.equals(whatWeFailedToBuild))
            {
                D.ebugPrintln("!!! BUILD REQUEST FOR A CITY " + Integer.toHexString(cityToBuildCoord[0]) + " !!!");
                client.buildRequest(game, SOCPlayingPiece.CITY);
            } else {
                // We already tried to build this.
                cancelWrongPiecePlacementLocal(whatWeWantToBuild);
